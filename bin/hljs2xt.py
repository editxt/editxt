...
Usage:
    syntaxgen.py [--overwrite] [-v...] <hljs-file> <editxt-file>
    syntaxgen.py -h | --help
...
def transform_syntax(data, definitions, name=None):
    ...
    for item in data._get("contains", []):
        ...
        for item in items:
            ...
            if "end" in item or "endsWithParent" in item \
...
#        else:
    ...
    if "starts" in item and item.starts:
        ...
        end = transform_end(item._parent)
        if not end.is_look_ahead():
            end = end.look_ahead()
...
def transform_end(item):
    if "end" in item and item.end:
        end = regex(item.end)
    elif "endsWithParent" not in item:
        end = RE(r"\B|\b")
    else:
        end = RE("")
    if end and (item._get("excludeEnd") or item._get("returnEnd")):
        end = end.look_ahead()
    if "endsWithParent" in item and "end" in item._parent:
