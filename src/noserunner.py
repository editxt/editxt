...
log = logging.getLogger("noserunner")
...
def run(argv=None):
    ...
    while True:
        ...
        if mod.startswith("editxt."):
            mod = ".".join("test_" + p for p in mod.rsplit(".", 1))
...
def augment_tests(testmods):
    ...
    """
    def mods():
        import test_editxt.test_application as mod; yield mod
        import test_editxt.test_editor as mod; yield mod
        import test_editxt.test_project as mod; yield mod
        import test_editxt.test_document as mod; yield mod
    def wrap(test, mod):
        def func():
            def dc_test(when):
